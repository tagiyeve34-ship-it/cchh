...
class CallActivity : AppCompatActivity() {
    ...
    private fun boot(){Api.get("ice_config.php",Session.token(this)){ok,raw->val ice=if(ok)runCatching{Gson().fromJson(raw,IceConfigResponse::class.java).iceServers}.getOrDefault(emptyList()) else emptyList();runOnUiThread{rtc=WebRtcClient(this,video,ice,{sendIce(it)},{st->runOnUiThread{b.callStatus.text=when(st){"CONNECTED"->"Qoşuldu";"DISCONNECTED"->"Bağlantı kəsildi";"FAILED"->"Bağlantı alınmadı";else->st}}});if(video){rtc?.attachLocal(b.localVideo)};h.post(poll);if(!incoming)startOutgoing() else b.callStatus.text="Gələn zəng"}}}
